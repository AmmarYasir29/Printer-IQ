...
import { comparePassword, errRes, getOTP, hashMyPassword, okRes } from "../helper/tools";
...
export default class controller {
    ...
    await image.mv(pathpdf, async function(err) {
      ...
      client.upload(pathpdf).then(data => {
        ...
        pdf(dataBuffer).then(async function(data) {
          ...
          let papers: any;
          try {
            papers = await Invoice.create({...req.body,countPainter:pages,fileurl:url,userId:req.user.id});
            await papers.save();
    ...
    static async register(req: Request, res: Response): Promise<object> {
        ...
        if (!phoneObj.isNumber)
          return errRes(res, `phoneInvalid${req.body.phone}`);
